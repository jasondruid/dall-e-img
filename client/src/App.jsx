...
const App = () => {
  ...
  const showOnTop = currentImage === "" ? { display: "flex", flexWrap: "wrap", height: "100%" } : { overflow: "hidden", position: "relative", width: "100%", maxHeight: "100%",minHeight:"100vh" }
  const [toggle, setToggle] = useState(false);
  const backgroundRef = useRef(null);
  console.log("This website has been created by Jason Ancona");

  useEffect(() => {
    if (currentImage !== "") {
      console.log("back");
      backgroundRef.current?.scrollTo({ top: 0, behavior: "auto" });
    }
  }, [currentImage])
  ...
  return (
    <>
      <ToastContainer />
      <div className={`${count} w-full h-full`} style={showOnTop}>
        <div ref={backgroundRef} style={{ height: "100%", width: "100%", zIndex: 999, position: "absolute", alignItems: "center", justifyContent: "center", display: currentImage === "" ? "none" : "flex" }} className="backdrop-blur-sm bg-white/30" onClick={() => { dispatch(setPhoto("")) }}>
          <div style={{ height: "100%", width: "100%", position: "relative", padding: "3%", display: "flex", overflow: 'hidden' }} >
        ...
        <BrowserRouter>
          <header className='w-full sticky top-0 z-30 flex dark:bg-gray-800 bg-white justify-between items-center  sm:px-8 px-4 py-4 border-b border-b-[#e6ebf4] dark:border-b-gray-800'>
            ...
            <div>
              ...
              <img className={`w-[28px] h-[28px] object-contain ml-3 mr-1 inline sm:hidden ${checked ? "" : "svg-white"}`} onClick={() => setToggle(prev => !prev)} alt="menu" src={toggle ? close : menu} />

              <div></div>
              <div className={`${toggle ? 'flex' : "hidden"} p-6 bg-gradient-to-r from-[#6d6d6d] to-[#11101d] dark:from-[#6d6d6d] dark:to-[#5a656e]  absolute top-20 right-0 mx-4 my-2 min-w[140px] rounded-xl sidebar`}>
          ...
          <main className='sm:p-8 px-4 py-8 w-full bg-[#f9fafe] min-h-[calc(100vh-61px)] sm:min-h-[calc(100vh-73px)]  dark:bg-gray-900 '>
            <Routes>
